...
title 'Summary statistics: Incidents per Capita by year 2020-2023';
proc means data=lib.poptotal nmiss min q1 mean median q3 max std maxdec=2;
	class year;
	var incidents_per_capita_100000;
run;

title 'Summary statistics: Incidents per Capita by County (2020-2023)';
proc means data=lib.poptotal nmiss min q1 mean median q3 max std maxdec=2;
	class county;
	var incidents_per_capita_100000;
run;
...
title 'Fails normality test';
proc univariate data=lib.poptotal normaltest plots;
	var incidents_per_capita_100000;
run;

title 'Year*incidents_per_capita: Exact Kruskal-Wallis Test with Monte Carlo estimation';
proc npar1way data=lib.poptotal;
    class year;
    var incidents_per_capita_100000;
    exact wilcoxon normal savage median edf ks hl/ mc seed=43;
run;

/*
title 'Plot: County populations from 2020-2023';
proc sgplot data=lib.poptotal noautolegend;
	series x=year y=incidents_per_capita_100000/ group=county curvelabel groupdisplay=cluster;
	xaxis type=discrete;
